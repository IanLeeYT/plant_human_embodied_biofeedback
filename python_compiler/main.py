...
def reg_calibrate_array(arr, dt, transfer_rate):
  ...
  """
    ...
    reg_arr = np.nanmean(np.pad(arr.astype(float),
                                (0,
                                 (grouping - arr.size % grouping) % grouping),
...
def dynamic_amplification(arr, pmx):
  ...
  """
    ...
    for i in range(len(arr)):
        if pmids > arr[i] > threshold and arr[i] > arr[i -
                                                       1] and arr[i] > arr[min(
                                                           i + 1,
                                                           len(arr) - 1)]:
            arr_res[i] = pmids
  ...
  """
    ...
    while cont:
        ...
        if user == "end" or user == "quit":
            ...
            if file_name not in audio_catalog:
                print("...Loading...\n")
                arr = audio_interpreter.audio_to_array(user, sr_freq)
                max_arr = max_array(arr, dt, sr_freq)
